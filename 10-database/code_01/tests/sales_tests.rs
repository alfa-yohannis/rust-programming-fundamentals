...
async fn test_sales_order_with_details() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let details = vec![
        SalesOrderDetail {
            ...
            line_num: 1,
            item_code: "0000000010".to_string(),
            quantity: 2.0,
            unit: Some("pcs".to_string()),
            unit_price: 50.0,
            currency: "USD".to_string(), // New field added
        },
        SalesOrderDetail {
            id: 0, // This will be auto-generated by the database
            order_code: new_order.code.clone(),
            line_num: 2,
            item_code: "0000000020".to_string(),
            quantity: 5.0,
            unit: Some("pcs".to_string()),
            unit_price: 75.0,
            currency: "USD".to_string(), // New field added
    ...
    for (i, detail) in details.iter().enumerate() {
        ...
        assert_eq!(fetched_details[i].item_code, detail.item_code);
        ...
        assert_eq!(fetched_details[i].currency, detail.currency); // New assertion added
    }
    ...
    updated_details[0].currency = "EUR".to_string(); // Update currency for the first detail
    ...
    assert_eq!(
        updated_fetched_details[0].currency,
        updated_details[0].currency
    ); // New verification for updated currency
